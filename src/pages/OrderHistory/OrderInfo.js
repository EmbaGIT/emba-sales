...
const OrderInfo = (props) => {
  const bankInfo = props.orderState.find(
    (o) => o.id === props.info.id
  )?.bankInfo;
  ...
  const [getFullInfo, { data: customer_full_info }] = useLazyQuery(
    FULL_INFO_QUERY,
    {
      context: { headers: { authorization: `Bearer ${authCtx.token}` } },
      onCompleted: () => {
      ...
      buttons: [
        {
          ...
          onClick: () => {
            ...
            const promises = info.goods.map(
              (good) =>
                new Promise((resolve) => {
                  if (good.color_id) {
                    get(
                      `${getHost("files", 8089)}/api/image/resource?brand=${
                        good.brand
                      }&color=${good.color_id}&category=${
                        good.category_id
                      }&bucket=emba-store-images&parent=${
                        good.parent_id
                      }&product=${good.product_id}&isBanner=true`
                    ).then((file) => {
                      resolve({
                        ...
                        discount_price:
                          good.product_price -
                          (good.product_price * good.product_discount) / 100,
  ...
  const handleOrderInfo = (resOrderInfo) => {
    ...
    if (resOrderInfo.client_delivery_city_code) {
      get(
        `${getHost("sales", 8087)}/api/city/code/${
          resOrderInfo.client_delivery_city_code
        }`
      ).then((city) => {
        setOrderInfo((prevstate) => ({
          ...prevstate,
    ...
    resOrderInfo.goods.map((item) => {
      ...
      discount_total_price +=
        ...
        (item.product_quantity * item.product_price * item.product_discount) /
          100;
      get(`/products/uid/${item.product_uid}`).then((res) => {
        ...
        setOrderInfo((prevstate) => ({
          ...
          passport_series: resOrderInfo.client_passport_series
            ? resOrderInfo.client_passport_series
            : "AZE",
          ...
          dsmf_barcode: resOrderInfo.dsmf_barcode,
  ...
  const handleInputChange = (type, value) => {
    ...
    if (type === "select_city") {
      ...
      alldata = {
        ...alldata,
        bank_cash:
          Math.round(((orderInfo.discountPrice * 2) / 100) * 100) / 100,
        paymentType: 1,
      };
    ...
    if (handleNameSurnameValidation()) {
      getAvailableCustomer({
        variables: {
          ...
          serial: orderInfo.identifierNumber
            ? orderInfo.identifierNumber
            : null,
    ...
    if (handleValidation()) {
      ...
      const postData = {
        ...
        client_new_phone: customerRefactoringInfo.mobile_phone
          ? oldCustomerInfo.mobile_phone ===
            customerRefactoringInfo.mobile_phone
            ? 0
            : 1
          : 0,
      ...
      post(
        `${getHost("sales", 8087)}/api/order/update/${orderInfo.id}`,
        postData
      )
        .then((res) => {
          setIsSending(false);
      ...
      <div className="row">
        <div className="col-lg-12">
          ...
          <div className="tab-content" id="ex1-content">
              ...
              <table className="table table-striped table-hover table-bordered">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <th colSpan={orderInfo?.status === "ORDERED" ? 4 : 5}>
                      {orderInfo.totalPrice.toFixed(2)} AZN
                    </th>
                  </tr>
                  <tr>
                    <th>
                    ...
                    <th colSpan={orderInfo?.status === "ORDERED" ? 4 : 5}>
                      {orderInfo.discountPrice.toFixed(2)} AZN
                    </th>
                  </tr>
                </tbody>
              </table>
            ...
            >
              {orderInfo?.status !== "ORDERED" && <h6>Müştəri məlumatları</h6>}
              <div className="input-group row mb-3">
                ...
                <div className="col-md-4">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange("finCode", e.target.value)
                    }
                  />
                </div>
                  ...
                  <div className="row">
                    <div className="col-md-4 pe-0">
                      <select
                        ...
                        disabled={orderInfo?.status === "ORDERED"}
                        value={
                          orderInfo && orderInfo?.passport_series
                            ? orderInfo?.passport_series
                            : ""
                        }
                        onChange={(e) =>
              ...
              <div className="row mb-3">
                <div className="col-md-12">
                  <label>DSMF Barcode</label>
                  <input
                    className="form-control"
                    disabled={
                      orderInfo?.status === "ORDERED" ||
                      isRefactorDisabled.dsmf_barcode
                    }
                    onChange={(e) =>
                      handleInputChange("dsmf_barcode", e.target.value)
                    }
                    value={orderInfo && orderInfo?.dsmf_barcode}
                  />
                </div>
              </div>
              <div className="row mb-3">
                <div className="col-md-6">
                  ...
                  <BirthDateDatepicker
                    selectedDate={
                      orderInfo?.birthdate ? new Date(orderInfo?.birthdate) : ""
                    }
                    isDisabled={
                      orderInfo?.status === "ORDERED" ||
                ...
                <div className="col-md-6">
                  ...
                  <Select
                    isDisabled={
                      orderInfo?.status === "ORDERED" || isRefactorDisabled.city
                    }
                    styles={selectStyles}
                    options={city}
                    value={
                      orderInfo && orderInfo?.city
                        ? [
                            {
              ...
              <div className="row mb-3">
                <div className="col-md-6">
                  ...
                  <DatePicker
                    ...
                    selected={
                      orderInfo?.payment_date
                        ? new Date(orderInfo?.payment_date)
                        : ""
                    }
                ...
                <div className="col-md-6">
                  ...
                  <DatePicker
                    ...
                    selected={
                      orderInfo?.delivery_date
                        ? new Date(orderInfo?.delivery_date)
                        : ""
                    }
                    onChange={(date) =>
            ...
            {bankInfo && (
              <div
                ...
                id="installment-info"
                role="tabpanel"
                aria-labelledby="installment-info"
              >
                <div>
                  <table className="table table-striped table-hover table-bordered">
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td>
                          {+bankInfo?.totalAmount - +bankInfo?.initialAmount}
                        </td>
                      </tr>
                      <tr>
        ...
        {props.info.status === "SAVED" && (
          <div>
            ...
            <button
              ...
              disabled={isSending}
              onClick={sendWishListOrder}
            >
              {isSending ? "Gözləyin" : "Sifarişi göndər"}
            </button>
          </div>
        )}
        <div className="btn btn-primary" onClick={props.onCloseModal}>
          Bağla
        </div>
      </div>
    </Modal>
