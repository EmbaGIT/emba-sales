...
const FULL_INFO_QUERY = gql`
  query fullInfo($uid: String) {
    search(criteria: { uid: { equals: $uid } }) {
      id
      uid
      name
      isActive
...
const Checkout = () => {
  ...
  const [
    ...
    { data: available_customer, loading: available_customer_loading },
  ...
  const [
    getFullInfo,
    { data: customer_full_info, loading: customer_full_loading },
  ] = useLazyQuery(FULL_INFO_QUERY, {
    context: { headers: { authorization: `Bearer ${authCtx.token}` } },
    fetchPolicy: "network-only",
    onCompleted: () => {
      if (customer_full_info) {
        ...
        customer_full_info.search[0].details.forEach((detail) => {
          if (detail.infoTypeField.field === "Birthdate") {
            ...
            get(
              `${getHost("sales", 8087)}/api/city/search?name.contains=${
                detail.fieldValue
              }`
            ).then((res) => {
      ...
      const birthdateString = customer_full_info?.search[0]?.details?.find(
        (detail) => detail?.infoTypeField?.field === "Birthdate"
      )?.fieldValue;

      const formattedDate = birthdateString
        .split(" ")[0]
        .split(".")
        .reverse()
        .join("-");
  ...
  useEffect(() => {
    get(`${getHost("sales", 8087)}/api/city/table?page=0&size=100`)
      .then((res) => {
        setIsFetchingData(false);
        setCity(
          res?.content?.map((city) => ({
            value: city.code,
            label: `${city.name}`,
          }))
        );
  ...
  useEffect(() => {
    ...
    if (cartCtx.savedId) {
      post(
        `${getHost("sales", 8087)}/api/order/search?id.equals=${
          cartCtx.savedId
        }`
      ).then((resOrderInfo) => {
        setCheckoutState((prevState) => ({
  ...
  const handleInputChange = (type, value) => {
    ...
    if (type === "birthdate") {
      ...
      checkoutState.items.find(
        (item) => item.id === value.id
      ).product_createsales = value.checked;
    } else if (type === `product_reserve${value.id}`) {
      // SHUOLD BE IMMUTABLE STATE CHANGE, BUT MUTABLE DONE INSTEAD
      checkoutState.items.find((item) => item.id === value.id).product_reserve =
        value.checked;
    } else if (type === "cash") {
      setPaymentType(0);
      setBankCommission(0);
      ...
      setPaymentType(1);
      setBankCommission(
        Math.round(((cartCtx.discountAmount * creditPercent) / 100) * 100) / 100
      );
      ...
      !isRefactorDisabled.isDisabled &&
        setCustomerRefactoringInfo((prevState) => ({
          ...prevState,
          [type]: value,
        }));
  ...
  const searchOnDatabase = () => {
    ...
    const validNameSurnameSearchQuery =
      (checkoutState.customerInfo.surname.length >= 3 ||
        checkoutState.customerInfo.name.length >= 3) &&
      (checkoutState.customerInfo.surname.length !== 0 ||
        checkoutState.customerInfo.name.length !== 0);

    if (handleNameSurnameValidation() && validNameSurnameSearchQuery) {
      const { surname, name, patronymic, identifierNumber, finCode } =
  ...
  const handleValidation = () => {
    ...
    !checkoutState.customerInfo.surname
      ? setFormValidation((prevState) => ({
          ...prevState,
          surname: false,
        }))
    ...
    !checkoutState.customerInfo.other_phone
      ? setFormValidation((prevState) => ({
          ...prevState,
          other_phone: false,
        }))
    ...
    !checkoutState.customerInfo.city.code
      ? setFormValidation((prevState) => ({
          ...prevState,
          city: false,
        }))
      : setFormValidation((prevState) => ({ ...prevState, city: true }));
    !checkoutState.customerInfo.client_pur &&
    checkoutState.customerInfo.client_pur !== 0
      ? setFormValidation((prevState) => ({
          ...prevState,
          client_pur: false,
        }))
      : setFormValidation((prevState) => ({ ...prevState, client_pur: true }));
    !checkoutState.customerInfo.client_inter &&
    checkoutState.customerInfo.client_inter !== 0
      ? setFormValidation((prevState) => ({
          ...prevState,
          client_inter: false,
        }))
    ...
    !checkoutState.customerInfo.gender &&
    checkoutState.customerInfo.gender !== 0
      ? setFormValidation((prevState) => ({
          ...prevState,
          client_gender: false,
        }))
  ...
  const sendOrder = (status) => {
    ...
    if (status === "SAVED" && handleValidation()) {
      ...
      post(`${getHost("sales", 8087)}/api/order/wishlist`, order_data)
        .then((res) => {
                ...
                <tbody className="">
                  {checkoutState.items?.map((product) => (
                    <tr key={product.id}>
                      ...
                      <td>
                        {(product.amount * product.discount_price).toFixed(2)}{" "}
                        AZN
                      </td>
                    </tr>
                  ))}
              ...
              <div className="d-flex">
                <span className="form-check">
                  <input
                    ...
                    id="cash"
                    onChange={(e) =>
                      handleInputChange("cash", e.target.checked)
                    }
                ...
                <span className="form-check ms-3">
                  <input
                    ...
                    id="credit"
                    onChange={(e) =>
                      handleInputChange("credit", e.target.checked)
                    }
              ...
              <div className="d-flex">
                <span className="form-check">
                  <input
                    ...
                    id="byStore"
                    onChange={(e) =>
                      handleInputChange("byStore", e.target.checked)
                    }
                    checked={!!(deliveryType === 0)}
                  />
                  <label className="form-check-label" htmlFor="byStore">
                    Mağaza
                  </label>
                </span>
                <span className="form-check ms-3">
                  <input
                    ...
                    id="byCustomer"
                    onChange={(e) =>
                      handleInputChange("byCustomer", e.target.checked)
                    }
                    checked={!!(deliveryType === 1)}
                  />
                  <label className="form-check-label" htmlFor="byCustomer">
                    Müştəri
                  </label>
                </span>
              </div>
          ...
          <div className="card-footer">
            <ul className="ps-0">
              ...
              <li className="d-flex align-content-center justify-content-between mb-2">
                ...
                <div className="float-end">
                  <strong>{bankCommission} AZN</strong>
                </div>
              </li>
            </ul>
          </div>
          ...
          <div className="card-body">
            ...
            <div className="input-group row mb-3">
                ...
                (checkoutState.customerInfo.surname.length !== 0 ||
                  ...
                  <div className="col-12">
                    <small className="text-danger">
                      Axtarış üçün ad və ya soyad xanasına ən azı 3 simvol daxil
                      edin.
                    </small>
                  </div>
                )}
              ...
              <div className="col-md-4 mb-2">
                ...
                <input
                  ...
                  value={
                    checkoutState.customerInfo &&
                    checkoutState.customerInfo?.patronymic
                  }
                  onChange={(e) =>
              ...
              <div className="col-md-4">
                ...
                <input
                  ...
                  value={
                    checkoutState.customerInfo &&
                    checkoutState.customerInfo?.finCode
                  }
                ...
                <div className="row">
                  <div className="col-md-4 pe-0">
                    <select
                      ...
                      value={
                        checkoutState.customerInfo &&
                        checkoutState.customerInfo?.passport_series
                          ? checkoutState.customerInfo?.passport_series
                          : ""
                      }
                      onChange={(e) =>
                        handleInputChange("passport_series", e.target.value)
                      }
                  ...
                  <div className="col-md-8">
                    <InputMask
                      ...
                      value={
                        checkoutState.customerInfo &&
                        checkoutState.customerInfo?.identifierNumber
                      }
                      onChange={(e) =>
                        handleInputChange("identifierNumber", e.target.value)
                      }
                    />
                  </div>
                </div>
              ...
              <div className="col-md-3 d-flex align-items-end">
                <div className="btn btn-primary" onClick={searchOnDatabase}>
                  Axtar
                </div>
              </div>
            </div>
            ...
            {customerSearch &&
              (availableCustomer?.length ? (
                <div>
                  {!isNewCustomer && availableCustomer?.length > 1 ? (
                    <>
                      <label className={customerSelected ? "" : "text-danger"}>
                        Zəhmət olmasa müştərini siyahıdan seçin:
                      </label>
                      <select
              ...
              <div className="mb-3">
                <div className="form-check">
                  <input
                    ...
                    id="refactorInfo"
                    onChange={(e) =>
                      handleInputChange("refactor", e.target.checked)
                    }
            ...
            {customerSearch && (customerSelected || isNewCustomer) && (
              <>
                <div className="row mb-3">
                  <div className="col-md-6">
                    ...
                    <BirthDateDatepicker
                      selectedDate={
                        checkoutState.customerInfo.birthdate
                          ? new Date(checkoutState.customerInfo.birthdate)
                          : ""
                      }
                      isDisabled={
                        isRefactorDisabled.birthdate &&
                        availableCustomer?.length &&
                        !isNewCustomer
                      }
                    ...
                    >
                      ...
                      {checkoutState.customerInfo.birthdate &&
                        !formValidation.isYounger && (
                          <small className="text-danger">
                            Sifarişin qəbulu üçün minimum yaş həddi 16
                            olmalıdır.
                          </small>
                        )}
                    </div>
                  ...
                  <div className="col-md-6">
                    <label>
                      Şəhər<span className="text-danger">*</span>
                    </label>
                    <Select
                      isDisabled={
                        isRefactorDisabled.city &&
                        availableCustomer?.length &&
                        !isNewCustomer
                      }
                      styles={selectStyles}
                      ...
                      components={(props) =>
                        NoOptionsMessage(props, "Şəhər tapılmadı.")
                      }
                      onChange={(value) =>
                        handleInputChange("select_city", value)
                      }
                ...
                <div className="row mb-3">
                  <div className="col-md-6">
                    <label>
                      Mobil telefon<span className="text-danger">*</span>
                    </label>
                    <InputMask
                      mask="(+\9\9499) 999-99-99"
                      className="form-control"
                      disabled={
                        isRefactorDisabled.mobile_phone &&
                        availableCustomer?.length &&
                        !isNewCustomer
                      }
                      onChange={(e) =>
                  ...
                  <div className="col-md-6">
                    ...
                    <InputMask
                      ...
                      className="form-control"
                      disabled={
                        isRefactorDisabled.other_phone &&
                        availableCustomer?.length &&
                        !isNewCustomer
                      }
                      onChange={(e) =>
                        handleInputChange("other_phone", e.target.value)
                      }
                      value={
                        checkoutState.customerInfo &&
                        checkoutState.customerInfo.other_phone
                    ...
                    <div className="d-flex">
                      <span className="form-check">
                        <input
                          ...
                          id="male"
                          onChange={(e) =>
                            handleInputChange("male", e.target.checked)
                          }
                      ...
                      <span className="form-check ms-3">
                        <input
                          ...
                          id="female"
                          onChange={(e) =>
                            handleInputChange("female", e.target.checked)
                          }
                ...
                <div className="row mb-3">
                  <div className="col-md-6">
                    <label>
                      Alışın məqsədi<span className="text-danger">*</span>
                    </label>
                    <Select
                      styles={selectStyles}
                      ...
                      components={(props) =>
                        NoOptionsMessage(
                          props,
                          "Alışın məqsədi üçün seçimlər tapılmadı."
                        )
                      }
                      onChange={(value) =>
                        handleInputChange("select_client_pur", value.value)
                      }
                  ...
                  <div className="col-md-6">
                    <label>
                      Məlumat mənbəyi<span className="text-danger">*</span>
                    </label>
                    <Select
                      styles={selectStyles}
                      ...
                      components={(props) =>
                        NoOptionsMessage(
                          props,
                          "Məlumat mənbəyi üçün seçimlər tapılmadı."
                        )
                      }
                      onChange={(value) =>
                        handleInputChange("select_client_inter", value.value)
                      }
                ...
                <div className="row mb-3">
                  <div className="col-md-6">
                    <label>
                      Ödəniş tarixi<span className="text-danger">*</span>
                    </label>
                    <DatePicker
                  ...
                  <div className="col-md-6">
                    <label>
                      Çatdırılma tarixi<span className="text-danger">*</span>
                    </label>
                    <DatePicker
                      onChangeRaw={(e) => e.preventDefault()}
                      dateFormat="yyyy-MM-dd"
                ...
                <div className="row mb-3">
                  <div className="col-md-12">
                    ...
                    <textarea
                      ...
                      value={
                        checkoutState.customerInfo &&
                        checkoutState.customerInfo.note
                      }
                    />
                  </div>
                </div>
