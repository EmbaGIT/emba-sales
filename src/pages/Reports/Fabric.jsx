...
const Fabric = () => {
    ...
    const paginate = (n) => {
        setPage(+n.selected);
        history.push(`/reports/fabric/${n.selected}/10`);
    }

    const onPageSizeChange = (n) => {
        setPageSize({value: n.value, label: n.value});
        history.push(`/reports/fabric/${page}/${n.value}`);
    }

    useEffect(() => {
        const user = getUser();
        setIsFetching(true);
        get(`${getHost('report', 8091)}/api/fabric?page=${page}&size=${pageSize.value}&filter=${search}`).then((res) => {
            setFabric(res)
            setIsFetching(false);
        }).catch((err) => {
            console.log("err", err);
                        ...
                        <div className="table-responsive">
                            <table className="table bordered striped">
                                ...
                                <tr>
                                    ...
                                    <th scope='col'>Masallı divan-kreslo material anbarı</th>
                                    <th scope='col'>Masallı yumşaq mebel yarımfabrikat anbarı</th>
                                ...
                                {fabric.items?.map((good, i) => (
                                    <tr key={i}>
                                        ...
                                        <td><span
                                            className="cursor-pointer text-primary font-weight-bolder">{good?.fabric}</span>
                                        </td>
                                        <td>{good?.couchAndWingChairMaterialStock}</td>
                                        <td>{good?.semiFinishedProductStock}</td>
