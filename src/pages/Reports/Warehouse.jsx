...
const Warehouse = () => {
  ...
  const [pageSize, setPageSize] = useState({
    value: Number(params.pageSize),
    label: Number(params.pageSize),
  });
  const sizeOptions = [
    { value: 10, label: 10 },
    { value: 20, label: 20 },
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      get(
        `${getHost("erp/report", 8091)}/api/warehouse/${user?.uid}?page=${
          params.page
        }&size=${params.pageSize}&category=${
  ...
  const updateWarehouseInfo = () => {
    const user = getUser();
    setIsFetching(true);
    remove(`${getHost("erp/report", 8091)}/api/warehouse/${user?.uid}`)
      .then((res) => {
        get(
          `${getHost("erp/report", 8091)}/api/warehouse/${
            user?.uid
          }?page=0&size=10&category=${selectedCategory.value}&filter=${search}`
        )
          .then((res) => {
            setWarehouseInfo(res);
            setIsFetching(false);
  ...
  const addToCartHandler = async (uid, characteristics) => {
    ...
    const isPrice = product.characteristics.find(
    ...
    const parent = product.attributes.find(
      (a) => a.groups.name === "Model"
    ).name;
    const category = product.attributes
      .find((a) => a.groups.name === "Vidmebeli")
      .id.toString();
    const parent_id = product.attributes
      .find((a) => a.groups.name === "Model")
      .id.toString();
    const color_id = product.colors[0]?.id.toString() ?? "";
    const characteristic_uid =
      product.characteristics.find((c) => c.name === characteristics)?.uid ??
      "";
    const characteristic_code =
      product.characteristics.find((c) => c.name === characteristics)?.code ??
      "";
    ...
    const fileUrl = color_id
      ? `${getHost(
          "files",
          8089
        )}/api/image/resource?brand=${brand}&bucket=emba-store-images&category=${category}&parent=${parent_id}&product=${id}&color=${color_id}`
      : `${getHost(
          "files",
          8089
        )}/api/image/resource?brand=${brand}&bucket=emba-store-images&category=${category}&parent=${parent_id}&product=${id}`;
        ...
        {warehouseInfo.goods?.length && (
          <div className="col-12">
            <div className="d-flex justify-content-between align-items-center mb-4">
              <h4 className="fm-poppins flex-1 mb-0">
                {warehouseInfo.warehouse} mövcud məhsullar
              </h4>
              <div className="me-3">
                <button
                ...
                <tbody>
                  {warehouseInfo.goods
                    ?.sort((g1, g2) => {
                      if (
                        selectedCategory !== "" &&
                        selectedCategory.value === "UPHOLDSTERED"
                      ) {
                        return g1?.characteristicName?.localeCompare(
                          g2?.characteristicName
                        );
                      }
                    ...
                    .map((good, i) => (
                      <tr key={i}>
                        <td>
                          {+i + Number(page) * Number(pageSize.value) + 1}
                        </td>
                        <td>
                          <span className="cursor-pointer text-primary font-weight-bolder">
                            {good.productName}
                          </span>
                        </td>
