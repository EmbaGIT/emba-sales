      ...
      axios
        .get(
          `${getHost(
            "web",
            "8083"
          )}/api/products/web/search?name=${value}&page=0&size=5`,
          {
            headers: {
  ...
  return (
    <div className="position-relative h-100 d-flex align-items-center">
      <input
        ...
        placeholder="Ən az 3 simvol daxil edib axtarış edin"
        ref={inputBox}
        value={searchParam}
      ...
      <div className="live-search" style={{ display: searchDisplay }}>
        <ul>
          {searchResult.content?.length ? (
                  ...
                  <Link
                    to={
                      product.colors.length
                        ? `/product/${product.brand}/${product.categoryId}/${product.parentId}?color=${product.colors[0]?.id}`
            ...
            <span className="px-2">Axtarış nəticəsi tapılmadı</span>
          )}
        </ul>
        {searchResult?.totalElements > 5 && searchResult?.content?.length ? (
